    ...
    <div className="max-w-md mx-auto p-6 sm:p-8">
      <div className="text-center mb-6">
        ...
        <p className="text-white/45 text-sm">
          Abonnez-vous pour recevoir les horoscopes quotidiens de Maryse dans votre boite mail.
        </p>
        {subscriberCount > 0 && (
          <p className="text-violet-400/60 text-xs mt-2">
            Rejoignez {subscriberCount} autres abonnes
          </p>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <input
            ...
            required
            className="w-full pl-10 pr-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-violet-500/30 transition-all"
          />
        </div>
        ...
        <button
          ...
          className={`w-full py-3 px-6 rounded-lg font-semibold text-white transition-all ${
            status === 'loading'
              ...
              : 'bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 transform hover:scale-[1.02] active:scale-95'
          }`}
        >
          {status === 'loading' ? (
            ...
            <div className="overflow-hidden">
              <div className="flex items-center p-3 rounded-lg bg-green-900/20 border border-green-500/30">
                ...
                <span className="text-green-300 text-sm">
                  Merci ! Verifiez votre boite mail pour confirmer votre abonnement.
                </span>
              </div>
            </div>
          )}
      ...
      <p className="text-xs text-white/20 text-center mt-6">
        Vos donnees sont chiffrees et ne seront jamais partagees.
      </p>
    </div>
  );
}
