          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {Array.from({ length: daysInMonth }).map((_, dayIndex) => {
              ...
              return (
                <motion.button
                  key={day}
                  onClick={() => hasRitual && setSelectedDate(hasRitual[0])}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className={`
                    aspect-square rounded-xl 
                    transition-all duration-200
                    ${hasRitual ? 'cursor-pointer hover:shadow-lg' : 'cursor-default'}
                    ${isToday ? 'ring-2 ring-ancestral-gold' : ''}
                  `}
                  style={{
                  ...
                  <div className="relative w-full h-full flex items-center justify-center">
                    ...
                    {hasRitual && (
                      <div className="absolute top-1 right-1 flex">
                        {ritualCount > 0 && (
                          <span 
                            className="text-[10px] text-ancestral-gold font-bold"
                            style={{ textShadow: '0 0 4px rgba(210,105,30,0.8)' }}
                          >
